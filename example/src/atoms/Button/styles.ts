...
const getContainerStyles = (colors: Colors) =>
  StyleSheet.create({
    ...
    elevated: { ...elevation.level1, backgroundColor: colors.surface },
    disabled: { backgroundColor: `${colors.onSurface}3D` },
    outlined: { backgroundColor: colors.surface, borderColor: colors.outline, borderWidth: 1 },
    outlinedDisabled: { borderColor: `${colors.onSurface}1E` },
    text: { backgroundColor: undefined, paddingHorizontal: undefined, height: undefined, borderRadius: undefined },
  });

const getTitleStyles = (colors: Colors) =>
  StyleSheet.create({
    default: { marginHorizontal: 8, color: colors.primary },
    disabled: { color: `${colors.onSurface}60` },
    filled: { color: colors.onPrimary },
    tonal: { color: colors.onSecondaryContainer },
  });

export const useStyles = ({ disabled, fullWidth, variant = 'filled' }: Props) => {
